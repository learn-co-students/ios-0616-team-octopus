...
class MapViewController: UIViewController, CLLocationManagerDelegate, GMSMapViewDelegate, SWRevealViewControllerDelegate, WCSessionDelegate {
    ...
    func checkForWatchAvailability() -> Bool {
        if (session.isWatchAppInstalled == true) && (session.isPaired == true) && (session.isReachable == true) {
            return true
        }
        else {
            return false
...
//    }
    ...
    func mapView(_ mapView: GMSMapView, markerInfoWindow marker: GMSMarker) -> UIView? {
        ...
        mapView.camera = GMSCameraPosition(target: marker.position, zoom: currentZoomLevel, bearing: mapView.camera.bearing, viewingAngle: mapView.camera.viewingAngle)
